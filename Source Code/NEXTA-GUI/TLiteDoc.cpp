...
CTLiteDoc::CTLiteDoc()
{
	...
	AddNameIntoTimingPlanVector(0, 1440,"ALLDAY");

	m_CurrentDisplayTimingPlanName = "ALLDAY";
...
BOOL CTLiteDoc::SaveLinkData(LPCTSTR lpszPathName,bool bExport_Link_MOE_in_input_link_CSF_File, int SelectedLayNo)
{
	...
	{
		std::list<DTALink*>::iterator iLink;
		fprintf(st,"name,link_id,link_key,speed_sensor_id,count_sensor_id,from_node_id,to_node_id,link_type_name,direction,length,number_of_lanes,speed_limit,lane_capacity_in_vhc_per_hour,link_type,jam_density,wave_speed,demand_type_code,mode_code,network_design_flag,grade,geometry,original_geometry,");
...
BOOL CTLiteDoc::SaveNodeFile()
{
	...
	{
		std::list<DTANode*>::iterator iNode;
		fprintf(st, "name,node_id,control_type,control_type_name,cycle_length_in_second,x,y,geometry\n");
		for (iNode = m_NodeSet.begin(); iNode != m_NodeSet.end(); iNode++)
		{
			//		if((*iNode)->m_LayerNo == SelectedLayNo) 
			{
				...
				fprintf(st, "%s,%d,%d,%s,%d,%f,%f,\"<Point><coordinates>%f,%f</coordinates></Point>\"\n", 
...
BOOL CTLiteDoc::SaveProject(LPCTSTR lpszPathName, int SelectedLayNo)
{
	...
	//					DTANodeMovement movement = (*iNode)->m_MovementDataMap["ALLDAY"].m_MovementVector[movement_index];
...
void CTLiteDoc::ScanAMSTimingPlanCSVFile(LPCTSTR lpszFileName, int scenario_no = 0)
{
	...
	AddNameIntoTimingPlanVector(m_DemandLoadingStartTimeInMin , m_DemandLoadingEndTimeInMin,"ALLDAY");
	...
	if (parser.OpenCSVFile(lpszFileName))
	{
		while(parser.ReadRecord())
		{
			...
			for(int time = m_DemandLoadingStartTimeInMin ; time <m_DemandLoadingEndTimeInMin; time+=15)
			{
				...
				if(timing_plan_name.size () > 0 && timing_plan_name != "ALLDAY")
				{
...
//			parser_movement.GetValueByFieldName("timing_plan_name",timing_plan_name);
			...
			if(m_MovementPointerMap.find(label) != m_MovementPointerMap.end())
			{
				...
				//parser_movement.GetValueByFieldName ("GreenStartTime",pMovement->QEM_StartTime );
				//parser_movement.GetValueByFieldName ("GreenEndTime",pMovement->QEM_EndTime );

				//int CycleLength = 70;

				//if(parser_movement.GetValueByFieldName ("CycleLength",CycleLength ))
				//{

				//	if(pNode != NULL && CycleLength >=1 )
				//	{
				//		pNode ->m_CycleLengthInSecond = CycleLength;
				//	}
				//}

				//int Offset = 0;
				//if(parser_movement.GetValueByFieldName ("Offset",Offset ))
				//{

				//	if(pNode != NULL && Offset >=1 )
				//	{
				//		pNode ->m_SignalOffsetInSecond  = Offset;
				//	}


				//}

				//if(pMovement->QEM_Lanes >=1 && pMovement->QEM_EffectiveGreen < 6 &&( pNode->m_ControlType == m_ControlType_PretimedSignal || pNode->m_ControlType == m_ControlType_ActuatedSignal))
				//{
				//	pMovement->QEM_EffectiveGreen =  (int)( pNode->m_CycleLengthInSecond* pLink0->m_LaneCapacity / pLink0->m_Saturation_flow_rate_in_vhc_per_hour_per_lane);
				//}
	...
	for (iNode = m_NodeSet.begin(); iNode != m_NodeSet.end(); iNode++)
	{
		if((*iNode)->m_LayerNo == 0  && ((*iNode)->m_ControlType == m_ControlType_PretimedSignal || (*iNode)->m_ControlType == m_ControlType_ActuatedSignal)) 
		{
			...
			{
				if((*iNode)->m_MovementDataMap["ALLDAY"].m_MovementVector[m].QEM_Lanes >=1 && (*iNode)->m_MovementDataMap["ALLDAY"].m_MovementVector[m].QEM_EffectiveGreen  <=6 )
				{  // use default value


					DTALink* pLink0 = m_LinkNoMap[(*iNode)->m_MovementDataMap["ALLDAY"].m_MovementVector[m].IncomingLinkNo];


					(*iNode)->m_MovementDataMap["ALLDAY"].m_MovementVector[m].QEM_EffectiveGreen = 
...
int CTLiteDoc::ReadAMSSignalControlCSVFile(LPCTSTR lpszFileName)
{
	...
	//			pNode->m_CycleLengthInSecond = Offset;
	...
	for (iVehicle = m_VehicleSet.begin(); iVehicle != m_VehicleSet.end(); iVehicle++)
	{
		...
		for(int i= 2; i<pVehicle->m_NodeSize; i++)
		{
			...
			if(pLink0 !=NULL && pLink1!=NULL)
			{
				...
				//if(timing_plan.timing_plan_name != "ALLDAY")
				{
					DTATimingPlan timing_plan_all_day;

					timing_plan_all_day.timing_plan_name = "ALLDAY";
	...
	for (iNode = m_NodeSet.begin(); iNode != m_NodeSet.end(); iNode++)
	{
		for(unsigned int m = 0; m< (*iNode)->m_MovementDataMap["ALLDAY"].m_MovementVector .size(); m++)
		{
			...
			for(unsigned int j = 0; j< (*iNode)->m_MovementDataMap["ALLDAY"].m_MovementVector .size(); j++)
			{
				...
				{
					total_link_count+= (*iNode)->m_MovementDataMap["ALLDAY"].m_MovementVector[j].sim_turn_count ;
				}
...
bool CTLiteDoc::WriteSubareaFiles()
{
	...
	if(st!=NULL)
	{
		...
		fprintf(st,"congestion_index,link_id,from_node_id,to_node_id,direction,length_in_mile,number_of_lanes,speed_limit,lane_capacity_in_vhc_per_hour,link_type,jam_density,wave_speed,mode_code,grade,prohibited_u-turn,geometry\n");
		for (iLink = m_SubareaLinkSet.begin(); iLink != m_SubareaLinkSet.end(); iLink++)
		{
			if((*iLink)->m_AVISensorFlag == false)
			{
...
void CTLiteDoc::RunQEMTool(int NodeNumber = -1, bool bObservationFlag = true)
{
	...
	SaveMovementData(m_ProjectDirectory + "input_movement.csv", -1,bObservationFlag);
	...
	CCSVWriter SettingFile;
	if(SettingFile.Open(CString2StdString(QEM_setting_file)))
	{

		str.Format("%s\n",m_ProjectDirectory);
		SettingFile.WriteTextLabel (str);

		// timing plan to be optimized

		for(int tp = 0; tp< m_TimingPlanVector.size(); tp++)  // first loop for each timing plan
		{
			std::string timing_plan_name = m_TimingPlanVector[tp].timing_plan_name;  // fetch timing_plan (unique) name

			SettingFile.WriteTextLabel (timing_plan_name.c_str() );
			SettingFile.WriteTextLabel (";");
		}
		SettingFile.WriteTextLabel ("\n");
		...
		if(NodeNumber>=1)
		{
			...
			DTA_Phasing_Data_Matrix element = GetPhaseData(NodeNumber ,"ALLDAY");

			if(element.GetString (DTA_SIG_PHASE_VALUE, TIMING_RingType).Find("Single")!=-1)  // single ring
				SettingFile.WriteTextLabel ("single_ring\n");
			else
				SettingFile.WriteTextLabel ("dual_ring\n");

		}
		SettingFile.CloseCSVFile ();
	...
	ReadAMSMovementCSVFile( m_ProjectDirectory+"input_movement.csv");
			...
			for(int tp = 0; tp< m_TimingPlanVector.size(); tp++)  // first loop for each timing plan
			{
				...
				if((*iNode)->m_MovementDataMap.find(timing_plan_name) != (*iNode)->m_MovementDataMap.end())
				{
					for(unsigned int m = 0; m< (*iNode)->m_MovementDataMap[timing_plan_name].m_MovementVector .size(); m++)
					{
						...
						MovementFile.SetValueByFieldName  ("CycleLength",CycleLength );
						MovementFile.SetValueByFieldName  ("Offset",(*iNode)->m_SignalOffsetInSecond );
...
void CTLiteDoc::GenerateMovementShapePoints()
{
	...
	for (iNode = m_NodeSet.begin(); iNode != m_NodeSet.end(); iNode++)
	{
		...
		for (unsigned int i=0;i< pNode->m_MovementDataMap["ALLDAY"].m_MovementVector .size();i++)
		{
			...
			pNode->m_MovementDataMap["ALLDAY"].m_MovementVector[i].m_ShapePoints .clear ();
			...
			// 		GDPoint p1_new, p2_new, p3_new;
			pNode->m_MovementDataMap["ALLDAY"].m_MovementVector[i].m_ShapePoints.push_back(p1_new);
			pNode->m_MovementDataMap["ALLDAY"].m_MovementVector[i].m_ShapePoints.push_back(p2_new);
			pNode->m_MovementDataMap["ALLDAY"].m_MovementVector[i].m_ShapePoints.push_back(p3_new);
